...
TWaveShape TModel::setOscWaveshape(int osc, int value)
{
		...
		if (osc<OSCILLATORCOUNT) return oscwav[index];
		switch (osc)
		{
...
/*	
	...
	for (int ivoc = 0; ivoc<VOICECOUNT; ivoc++)
  {
        ...
        else if (Voices[ivoc].FReleased) p=ivoc;
  }
	if (velo>0)
    Voices[p].NoteOn(pitch); // TODO.. Replace 0 by p... */
  if (velo>0) Voices[0].NoteOn(pitch); else NoteOff();	 
}
